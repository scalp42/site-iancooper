...
var addemails, brushes, convertimg, convertpre, find, key, load, markup, posts, replaceall, routing, sanitize, show, time_zones, _i, _len, _ref;
...
$(function() {
  ...
  return load('posts/index.json', 'json', function(data) {
    var i, max, max_recent, now, post, _j, _k, _len1, _ref1;
    posts = [];
    _ref1 = data.posts;
    ...
    now = moment();
    for (i = _k = 0; 0 <= max ? _k < max : _k > max; i = 0 <= max ? ++_k : --_k) {
    ...
    return routing({
      fn: {
        latest: function(request) {
          ...
          now = moment();
          _results = [];
          for (i = _l = 0, _ref2 = posts.length; 0 <= _ref2 ? _l <= _ref2 : _l >= _ref2; i = 0 <= _ref2 ? ++_l : --_l) {
            if (posts[i].status === 'published' && now >= posts[i].moment) {
