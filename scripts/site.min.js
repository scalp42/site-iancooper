...
"-")+'"><code>'+g+"</code></a>")})};sanitize=function(d){return d.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")};replaceall=function(d,g){var l,h,k;h=0;for(k=g.length;h<k;h++)l=g[h],d=d.replace(l[0],l[1]);return d};vglize=function(d){return replaceall(" "+d+" ",[[/(\W)([A-Z_]+)(\W)/g,'$1<span class="vgl-keyword">$2</span>$3'],[/^\s/,""],[/\s$/,""]])};markup=function(d){null==window.Converter&&(window.Converter=new Markdown.Converter);return window.Converter.makeHtml(d)};
load=function(d,g,l){return $.ajax({url:d,dataType:g,error:function(){return console.dir(arguments)},success:function(d){return l(d)}})};find=function(d){return _.find(posts,function(g){return g.slug===d})};
